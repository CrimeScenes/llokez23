...
-- Function to create a new 2D Line
...
-- Function to make lines visible or not
...
-- Function to update color of lines
...
-- Function to update the box around the target player
local function updateBox(targetPlayer)
    ...
    BoxLibrary[targetPlayer] = Library
    
    local oripart = Instance.new("Part")
    oripart.Parent = game:GetService("Workspace")
    oripart.Transparency = 1
    oripart.CanCollide = false
    oripart.Size = Vector3.new(1, 1, 1)
    oripart.Position = Vector3.new(0, 0, 0)
    
    -- Updater function for drawing the box
    local function Updater()
        local c 
        c = game:GetService("RunService").RenderStepped:Connect(function()
            if targetPlayer.Character and targetPlayer.Character:FindFirstChild("Humanoid") and targetPlayer.Character:FindFirstChild("HumanoidRootPart") and targetPlayer.Character.Humanoid.Health > 0 then
                local Hum = targetPlayer.Character
                ...
                if vis then
                    ...
                    -- Change box color based on team settings
                    if Settings.Team_Check then
                        if targetPlayer.TeamColor == Players.LocalPlayer.TeamColor then
                    ...
                    if Settings.Team_Color then
                        Colorize(Library, targetPlayer.TeamColor.Color)
                    end

                    -- Update the box lines
                    ...
                    if Settings.Autothickness then
                        local distance = (Players.LocalPlayer.Character.HumanoidRootPart.Position - oripart.Position).magnitude
                ...
                Vis(Library, false)
                if not game:GetService("Players"):FindFirstChild(targetPlayer.Name) then
...
-- Function to update tracer for the target player
...
-- Main loop to update visuals only for the target player
RunService.RenderStepped:Connect(function()
    if visualsActive then
        if TargetPlayer then
            updateBox(TargetPlayer)
